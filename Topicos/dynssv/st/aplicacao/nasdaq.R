# Obtenha os dados do nasdaq
nasdaq = quantmod::getSymbols('^IXIC', 
                            src = 'yahoo', 
                            from = '1988-07-05', to = '2003-07-03',
                            auto.assign = FALSE)
nasdaq = na.omit( nasdaq )
nasdaq = data.frame( nasdaq )
dates = as.Date( row.names( nasdaq ), '%Y-%m-%d' )
nasdaq = nasdaq[, 'IXIC.Adjusted']
#View(nasdaq)
T = length( nasdaq )
log.ret = 100 * ( log( nasdaq[2:T] ) - log( nasdaq[1:(T-1)] ) )
T = length( log.ret )
# Plots
library(ggplot2)
