...
#Rcpp::sourceCpp("parallel_pdf_s.cpp")
Rcpp::sourceCpp("~/HMMnew/svm_s/parallel_pdf_s2.cpp")
################################################################################
...
g_dim=6e3
gmax=2.5
y = svms.sim(mu=mu,phi=phi,sigma=sigma,nu=nu,beta=beta,y0=y0,g_dim=g_dim)
plot(y$y, type='l')
################################################################################
Results = times  = list()
npd = rep(0, 4)
...
for(m in c(50, 100, 150, 200)){
  ...
  while( any(eigenvalues <= 0) && cont<5 ){
    ...
    optim_res = svms.map(y=y1,m=m, parvect0=parvect0, y0=y0, gmax=gmax)
  ...
  if(any(eigenvalues <= 0)){
    ...
    # Weigths Evaluation
    n=1e3
    X=rmvnorm(n, mean=map, sigma=H1)
    Weigth=array(0,dim=c(n,1))
    for(j in 1:n){
      Weigth[j,1] = exp(k
                        -dmvnorm(X[j,],mean=map, sigma=H1, log=TRUE) 
                        -svms.posterior(X[j,],y=y1,y0=y0,m=m,gmax=gmax)
      )
    }
    s = sum(Weigth)
    if((s != 0) && !is.nan(s)){
    ...
    Results[[i]] = ISdiag(Weigth=Weigth, X=X, knu=0)
    times[[i]] = as.numeric(Sys.time()-time, units='mins')
  }
  i = i + 1
}
save(Results, times, npd, file='sim1_s_1500.RData')
################################################################################
Results = times  = list()
npd = rep(0, 4)
i=1
...
save(Results, times, npd, file='sim1_s_3000.RData')
################################################################################
Results = times  = list()
npd = rep(0, 4)
i=1
...
save(Results, times, npd, file='sim1_s_6000.RData')
################################################################################