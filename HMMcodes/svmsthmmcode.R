...
sourceCpp("svmn.cpp")
###############################################################################
svmt.pn2pw <- function(beta,mu,phi,sigma,nu,alpha){
  lbeta1<- beta[1]
  ...
  lnu = log(nu-2)
  lalpha = alpha
  parvect <- c(lbeta1,lbeta2,lbeta3,lmu,lphi,lsigma,lnu,lalpha)
  return(parvect)
}
svmt.pw2pn <- function(parvect){
  ...
  nu = exp(parvect[7]) + 2
  alpha = parvect[8]
  return(list(beta=beta,mu=mu,phi=phi,sigma=sigma,nu=nu, alpha=alpha))
}
fillallprobs <- function(x,beg,beta,nu,alpha,y){
  #return( (1/beg)*dt((x-beta[1]-beta[2]*y-beta[3]*beg^2)/beg,df=nu) )
  return( (1/beg)*sn::dst( (x-beta[1]-beta[2]*y-beta[3]*beg^2)/beg,
                           alpha=alpha,
                           nu=nu) )
}
svmt.mllk <-function(parvect,y,y0,m,gmax){
  ...
  xx<-y
  yy<-c(y0,y[1:(ny-1)])
  allprobs <- outer(xx,sey,"fillallprobs",beta=p$beta,nu=p$nu,alpha=p$alpha,yy)
  delta <-dnorm(bs,p$mu,p$sigma/sqrt(1-p$phi^2))*intlen
  foo <- delta*allprobs[1,]
  lscale = mlogLn_Rcpp(allprobs,Gamma,foo,ny) #Rcpp function
  return(-lscale)
}
svmt.prior <-function(parvect){
  ...
  + log(dnorm(parvect[8],0,10))
  return(-lprior)  
}
svmt.posterior <-function(parvect,y,y0,m,gmax){
  return(svmt.mllk(parvect,y,y0,m,gmax)+svmt.prior(parvect))  
}
svmt.map <- function(y,y0,m,beta0,mu0,phi0,sigma0,nu0,alpha0,gmax){
  parvect <- svmt.pn2pw(beta=beta0,mu=mu0,phi=phi0,sigma=sigma0,nu=nu0,alpha0)
  mod <- optim(parvect,svmt.posterior,y=y,y0=y0,m=m,gmax=gmax,hessian=T)
  mode <- mod$par
  return(list(mode=mode,lpostsvmt=-mod$value,hessian=mod$hessian))
}
stsvmeansim <-function(y0,beta,mu,phi,sigma,nu,alpha,g_dim){
  y=array(0,dim=g_dim)  
  h=array(0,dim=g_dim)    
  h[1]=rnorm(1,mu,sigma/sqrt(1-phi^2))
  y[1]=beta[1]+beta[2]*y0+beta[3]*exp(h[1])+exp(h[1]/2)*sn::rst(1,alpha=alpha, nu=nu)#rt(1,df=nu,ncp=0)
  for (j in 2:g_dim){
    h[j]=mu+phi*(h[j-1]-mu)+rnorm(1,0,sigma)
    y[j]=beta[1]+beta[2]*y[j-1]+beta[3]*exp(h[j])+exp(h[j]/2)*sn::rst(1,alpha=alpha, nu=nu)#rt(1,df=nu,ncp=0)
  }
...
###############################################################################
set.seed(8547403)
simsvmt1 <- stsvmeansim(y0=y0,beta=betasim,
                        mu=mu,phi=phi,sigma=sigma,nu=nu,alpha=alpha,
                        g_dim=g_dim)
...
res.svmt200_4_s1 <- svmt.map(simsvmt1$y[1:s1],m=m,
                             ...
                             sigma0=sigma0,nu0=nu0,alpha0=alpha0,
                             y0=simsvmt1$y0,
                             gmax=gmax)
res.svmt200_4_s1
#################################################################
...
alpham = sum(X[,8]*Weigth)
################################################################
cred_int_b0 = weighted_quantile(X[,1], Weigth, c(0.025, 0.975))
cred_int_b1 = weighted_quantile((exp(X[,2])-1)/(exp(X[,2])+1), 
                                Weigth, c(0.025, 0.975))
cred_int_b2 = weighted_quantile(X[,3], Weigth, c(0.025, 0.975))
cred_int_mu = weighted_quantile(X[,4], Weigth, c(0.025, 0.975))
cred_int_phi = weighted_quantile( (exp(X[,5])-1)/(exp(X[,5])+1), 
                                  Weigth, c(0.025, 0.975))
cred_int_sigma = weighted_quantile(exp(X[,6]), 
                                   Weigth, c(0.025, 0.975))
cred_int_nu = weighted_quantile( (exp(X[,7])+2), 
                                 Weigth, c(0.025, 0.975))
cred_int_alpha = weighted_quantile(X[,8], Weigth, c(0.025, 0.975))
# Results
...
results = rbind(results,
                ...
                c(alpha, alpham, cred_int_alpha)
                )
row.names(results) = c('b0','b1','b2','mu','phi','sigma','nu', 'alpha')
