...
for(rep in 1:N){
  ...
  try({
    ...
    while( any(eigenvalues <= 0) && cont<5 ){
      ...
      optim_res = svms.map(y=y$y,m=m, parvect0=parvect0, y0=y0, gmax=gmax)
    ...
    if(any(eigenvalues <= 0)){
      ...
      for(j in 1:n){
        Weigth[j,1] = exp(k
                          ...
                          -svms.posterior(X[j,],y=y$y,y0=y0,m=m,gmax=gmax)
                          )
      }
      s = sum(Weigth)
      if((s != 0) && !is.nan(s)){
      ...
      reps[[rep]] = ISdiag(Weigth=Weigth, X=X, knu=0)
    } 
...
save(reps, times, npd, file = 's_1500_50.RData')
