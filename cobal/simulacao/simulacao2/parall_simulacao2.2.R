...
path = paste0( dir, 'modelos/pcp.stan')
...
# Usando foreach para paralelizar
result = foreach(it = 1:M, .packages = c('rstan')) %dopar% {
  ...
  summary = num_analisys(draws = theta, 
                                 names = c('b', 
                                           'mu', 
                                           'phi_h', 
                                           's_h', 
...
save( result, file = paste0( dir, 'simulacao/simulacao2/parall_simulacao2.2.RData') )
